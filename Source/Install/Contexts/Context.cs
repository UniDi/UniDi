...
//#endif
        ...
        {
            foreach (var binding in injectableMonoBehaviours.OfType<UniDiBinding>())
            {
                if (binding == null)
                {
                    continue;
            ...
            // We'd prefer to use GameObject.FindObjectsOfType<UniDiBinding>() here
            // instead but that doesn't find inactive gameobjects
            // TODO: Consider changing this
            // Maybe UniDiBinding could add itself to a registry class on Awake/OnEnable
            // then we could avoid calling the slow Resources.FindObjectsOfTypeAll here
            foreach (var binding in Resources.FindObjectsOfTypeAll<UniDiBinding>())
            {
                if (binding == null)
                {
                    continue;
                ...
                // This is necessary for cases where the UniDiBinding is inside a GameObjectContext
                ...
                {
                    InstallUniDiBinding(binding);
                }
            }
        }

        void InstallUniDiBinding(UniDiBinding binding)
        {
            if (!binding.enabled)
            {
                return;
            ...
            {
                Log.Warn("Found empty list of components on UniDiBinding on object '{0}'", binding.name);
                return;
            }
            ...
            foreach (var component in binding.Components)
            {
                ...
                if (component == null)
                {
                    Log.Warn("Found null component in UniDiBinding on object '{0}'", binding.name);
                    continue;
                }
