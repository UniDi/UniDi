...
namespace UniDi
{
    ...
    public class DiContainer : IInstantiator
    {
            ...
            IEnumerable<DiContainer> parentContainersEnumerable, bool isValidating)
        {
            ...
            _settings = UniDiSettings.Default;
            ...
            var settings = TryResolve<UniDiSettings>();
        ...
        // to bind it as well
        public UniDiSettings Settings
        {
            get { return _settings; }
            set
            {
                _settings = value;
                Rebind<UniDiSettings>().FromInstance(value);
            }
        }
        ...
        void ResolveDependencyRoots()
        {
            ...
            var instances = UniDiPools.SpawnList<object>();
            ...
            try
            {
                for (int i = 0; i < rootProviders.Count; i++)
                {
                    ...
                    using (var context = UniDiPools.SpawnInjectContext(this, bindId.Type))
                    {
            ...
            {
                UniDiPools.DespawnList(instances);
            }
        }
        ...
        void ValidateFullResolve()
        {
            ...
            foreach (var bindingId in _providers.Keys.ToList())
            {
                ...
                {
                    using (var context = UniDiPools.SpawnInjectContext(this, bindingId.Type))
                    {
            ...
            InjectContext context, List<ProviderInfo> buffer)
        {
            ...
            var allMatches = UniDiPools.SpawnList<ProviderInfo>();
            ...
            {
                UniDiPools.DespawnList(allMatches);
            }
        }
        ...
        ProviderInfo TryGetUniqueProvider(InjectContext context)
        {
            ...
            var localProviders = UniDiPools.SpawnList<ProviderInfo>();
            ...
            {
                UniDiPools.DespawnList(localProviders);
            }
        }
        ...
        {
            var buffer = UniDiPools.SpawnList<object>();
        ...
        public void ResolveAll(InjectContext context, List<object> buffer)
        {
...
#endif
            {
                ...
                var matches = UniDiPools.SpawnList<ProviderInfo>();
                ...
                {
                    UniDiPools.DespawnList(matches);
                }
            }
        }
        ...
        void CheckForInstallWarning(InjectContext context)
        {
            ...
            Log.Warn("UniDi Warning: It is bad practice to call Inject/Resolve/Instantiate before all the Installers have completed!  This is important to ensure that all bindings have properly been installed in case they are needed when injecting/instantiating/resolving.  Detected when operating on type '{0}'.  If you don't care about this, you can disable this warning by setting flag 'UniDiSettings.DisplayWarningWhenResolvingDuringInstall' to false (see docs for details on UniDiSettings).", rootContext.MemberType);
#endif
        }
        ...
        {
            using (var context = UniDiPools.SpawnInjectContext(this, type))
            {
        ...
        {
            using (var context = UniDiPools.SpawnInjectContext(this, id.Type))
            {
...
#endif
            {
                ...
                if (providerInfo == null)
                {
                    ...
                    if (memberType.IsArray && memberType.GetArrayRank() == 1)
                    {
                        ...
                        var results = UniDiPools.SpawnList<object>();
                        ...
                        {
                            UniDiPools.DespawnList(results);
                        }
                    }
        ...
        void SafeGetInstances(ProviderInfo providerInfo, InjectContext context, List<object> instances)
        {
            ...
            {
                var lookupId = UniDiPools.SpawnLookupId(provider, context.BindingId);
                ...
                {
                    UniDiPools.DespawnLookupId(lookupId);
                }
            }
            else
            {
            ...
            Type concreteType, bool autoInject, List<TypeValuePair> extraArgs, InjectContext context, object concreteIdentifier)
        {
...
#endif
            {
                ...
                var paramValues = UniDiPools.SpawnArray<object>(typeInfo.InjectConstructor.Parameters.Length);
                ...
                try
                {
                    for (int i = 0; i < typeInfo.InjectConstructor.Parameters.Length; i++)
                    {
                        ...
                        {
                            using (var subContext = UniDiPools.SpawnInjectContext(
                    ...
                    if (!IsValidating || allowDuringValidation)
                    {
                        //ModestTree.Log.Debug("UniDi: Instantiating type '{0}'", concreteType);
                        try
                        {
#if UNIDI_INTERNAL_PROFILING
                ...
                {
                    UniDiPools.DespawnArray(paramValues);
                }
            }
            ...
            InjectContext context, object concreteIdentifier, bool isDryRun)
        {
            ...
            for (int i = 0; i < typeInfo.InjectMethods.Length; i++)
            {
                ...
                var paramValues = UniDiPools.SpawnArray<object>(method.Parameters.Length);
            ...
            InjectContext context, out bool shouldMakeActive)
        {
            ...
            if (prefabWasActive)
            {
                initialParent = UniDiUtilInternal.GetOrCreateInactivePrefabParent();
            }
            else
#endif
            {
        ...
        public void InjectGameObject(GameObject gameObject)
        {
            ...
            var monoBehaviours = UniDiPools.SpawnList<MonoBehaviour>();
            try
            {
            ...
            {
                UniDiPools.DespawnList(monoBehaviours);
            }
        }
            ...
            GameObject gameObject, Type componentType, List<TypeValuePair> extraArgs, InjectContext context, object concreteIdentifier)
        {
            ...
            var injectableMonoBehaviours = UniDiPools.SpawnList<MonoBehaviour>();
            try
            {

                UniDiUtilInternal.GetInjectableMonoBehavioursUnderGameObject(gameObject, injectableMonoBehaviours);
            ...
            {
                UniDiPools.DespawnList(injectableMonoBehaviours);
            }
        ...
        {
            using (var context = UniDiPools.SpawnInjectContext(this, contractType))
            {
                context.Identifier = identifier;
                return Resolve(context);
            }
        ...
        {
            using (var ctx = UniDiPools.SpawnInjectContext(this, contractType))
            {
        ...
        public BindStatement StartBinding(bool flush = true)
        {
            ...
            var bindStatement = UniDiPools.SpawnStatement();
        ...
        public void BindInstances(params object[] instances)
        {
            for (int i = 0; i < instances.Length; i++)
            {
                ...
                Assert.That(!UniDiUtilInternal.IsNull(instance),
